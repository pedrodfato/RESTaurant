...
export class ReservasService {
  ...
  async create(createReservaDto: CreateReservaDto & { usuario_id: number }) {
    ...
    if (
      new Date(createReservaDto.data_reserva).getHours() < 7 ||
      new Date(createReservaDto.data_reserva).getHours() >= 23
    ) {
      throw new ConflictException(
        'Horário de reserva inválido. O restaurante funciona das 07:00 às 23:00.',
      );
    }

    const reserva = await this.db.transaction(async (tx) => {
      const [mesaAtualizada] = await tx
        .update(schema.mesas)
        .set({ status: 'reservada' })
        .where(
          and(
            eq(schema.mesas.id, createReservaDto.mesa_id),
            eq(schema.mesas.status, 'disponivel'),
          ),
        )
        .returning({
          id: schema.mesas.id,
      ...
      const [novaReserva] = await tx
        .insert(schema.reservas)
        .values({
          ...createReservaDto,
          status: 'ativa',
          data_reserva: new Date(createReservaDto.data_reserva),
        })
        .returning({
  ...
  findAll(role: string, usuario_id: number) {
    if (role === 'admin') {
      ...
      return this.db.query.reservas.findMany({
        where: eq(schema.reservas.usuario_id, usuario_id),
        columns: {
          usuario_id: false,
        },
      });
  ...
  async update(
    id: number,
    updateReservaDto: CancelarReservaDto & { usuario_id: number },
  ) {
